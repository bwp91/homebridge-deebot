...
export default class {
  ...
  async internalCustomAreaUpdate(accessory, value) {
    try {
      ...
      this.log.warn('[%s] command [%s] [%s]', accessory.displayName, command, typeof command);

      // Don't continue if no command for this number has been configured
      if (!command) {
        this.log.warn('[%s] returning as no command', accessory.displayName);
        return;
      }
      ...
      accessory.control.run(`SpotArea${accessory.context.commandSuffix}`, 'start', command);
      ...
      this.log.warn('[%s] end of handler');
    } catch (err) {
      // Catch any errors during the process
      const eText = parseError(err, [platformLang.errNotInit, platformLang.errNotReady]);
      this.log.warn('[%s] %s %s.', accessory.displayName, platformLang.speedFail, eText);
