  ...
  initialiseDevice(device) {
    try {
      ...
      for(let i = 1; i <= 15; i++) {
        ...
        const areaService = accessory.getService(`Area ${i}`) || accessory.addService(this.hapServ.Switch, `Area ${i}`, `area${i}`);
        if (!areaService.testCharacteristic(this.hapChar.ConfiguredName)) {
          accessory.log(`Setting area ${i} name to ${accessory.context.rawConfig[`areaNote${i}`] || `Area ${i}`}`);
          areaService.addCharacteristic(this.hapChar.ConfiguredName);
          areaService.updateCharacteristic(this.hapChar.ConfiguredName, accessory.context.rawConfig[`areaNote${i}`] || `Area ${i}`);
        }

        if (!areaService.testCharacteristic(this.hapChar.ServiceLabelIndex)) {
          areaService.addCharacteristic(this.hapChar.ServiceLabelIndex);
          areaService.updateCharacteristic(this.hapChar.ServiceLabelIndex, 3 + i);
        }

        areaService
          .getCharacteristic(this.hapChar.On)
          .updateValue(!!accessory.context[`selectedArea${i}`])
          .removeOnSet()
          .onSet(async (value) => {
            accessory.log(`${platformLang.predefinedArea} ${i} [${value ? platformLang.selected : platformLang.unselected}}]`);
            accessory.context[`selectedArea${i}`] = value;

            const isSpotArea = accessory.context.rawConfig[`areaType${i}`] === 'spotArea';
            let areasToUnselect = [];

            for(let j = 1; j <= 15; j++) {
              if(!accessory.context.rawConfig[`areaType${j}`] || j === i) {
                continue;
              }

              if (!isSpotArea || accessory.context.rawConfig[`areaType${j}`] !== 'spotArea') {
                areasToUnselect.push(j);
              }
            }

            areasToUnselect.forEach((area) => {
              accessory.getService(`Area ${area}`).updateCharacteristic(this.hapChar.On, false);
              accessory.context[`selectedArea${area}`] = false;
            });
          });
      }
  ...
  async internalCleanUpdate(accessory, value) {
    try {
      ...
      if(value) {
        ...
        // if any predefined area is selected, update command
        for (let i = 1; i <= 15; i++) {
          if (!accessory.context.rawConfig[`areaType${i}`]) {
            continue;
          }

          if (accessory.context[`selectedArea${i}`] !== true) {
            continue;
          }
          ...
          if (isSpotArea) {
            spotAreaIds = spotAreaIds
                .concat(accessory.context.rawConfig[`spotAreaIDs${i}`].split(','))
                .filter((value, index, array) => array.indexOf(value) === index);
            console.log({spotAreaIds});
          } else {
            customAreaCoordinates = accessory.context.rawConfig[`customAreaCoordinates${i}`];
            spotAreaIds = [];
          }

          // turn off area selection
          accessory.getService(`Area ${i}`).updateCharacteristic(this.hapChar.On, false);
          accessory.context[`selectedArea${i}`] = false;
        }
