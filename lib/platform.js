...
export default class {
  ...
  applyUserConfig(config) {
                ...
                switch (k) {
                  ...
                  case 'commandType15':
                    if (typeof v !== 'string' || v === '') {
                      ...
                      // Jsut take over the command type as it comes from a string enumeration.
                      this.deviceConf[id][k] = v;
                    }
                    break;
                  case 'spotAreaCommand1':
                  case 'spotAreaCommand2':
                  case 'spotAreaCommand3':
                  case 'spotAreaCommand4':
                  case 'spotAreaCommand5':
                  case 'spotAreaCommand6':
                  case 'spotAreaCommand7':
                  case 'spotAreaCommand8':
                  case 'spotAreaCommand9':
                  case 'spotAreaCommand10':
                  case 'spotAreaCommand11':
                  case 'spotAreaCommand12':
                  case 'spotAreaCommand13':
                  case 'spotAreaCommand14':
                  case 'spotAreaCommand15': {
                    if (typeof v !== 'string' || v === '') {
                      logIgnore(`${key}.${id}.${k}`);
                    } else {
                  ...
                  case 'customAreaCommand1':
                  case 'customAreaCommand2':
                  case 'customAreaCommand3':
                  case 'customAreaCommand4':
                  case 'customAreaCommand5':
                  case 'customAreaCommand6':
                  case 'customAreaCommand7':
                  case 'customAreaCommand8':
                  case 'customAreaCommand9':
                  case 'customAreaCommand10':
                  case 'customAreaCommand11':
                  case 'customAreaCommand12':
                  case 'customAreaCommand13':
                  case 'customAreaCommand14':
                  case 'customAreaCommand15': {
                    if (typeof v !== 'string' || v === '') {
                      logIgnore(`${key}.${id}.${k}`);
                    } else {
                      // Strip off everything else than figures and semicolons.
                      this.log.warn('Parsing custom area %s', v);
                      const stripped = v.replace(/[^-\d,]+/g, '');
                      this.log.warn('Result: %s', stripped);
                      if (stripped) {
                        this.deviceConf[id][k] = stripped;
                      } else {
                        logIgnore(`${key}.${id}.${k}`);
                  ...
                  case 'commandNote14':
                  case 'commandNote15':
                    // Just ignore the command notes as they are intented for user information during configuration only.
                    break;
                  case 'deviceId':
                  case 'label':
                    break;
    ...
    this.log.warn('Configuration is: %s', JSON.stringify(this.deviceConf));
  }
  ...
  async pluginSetup() {
    ...
    try {
      ...
      // TODO: Check if this need to be refactored to "predefined"
      ...
      const eText = parseError(err, [platformLang.disabled, platformLang.deviceListFail]);
  ...
  initialiseDevice(device) {
    try {
      ...
      // TODO: Refactor to "predefined"
      // Add the Eve characteristic for custom commands if any exist
      if (this.deviceConf[device.did].commandType1) {
        if (!cleanService.testCharacteristic(this.cusChar.CustomArea)) {
          this.log.warn('[%s] Adding predefined area characteristic.', accessory.displayName);
        ...
        cleanService.getCharacteristic(this.cusChar.CustomArea).onSet(async (value) => {
          await this.internalPredefinedAreaUpdate(accessory, value);
        });
      } else if (cleanService.testCharacteristic(this.cusChar.CustomArea)) {
        this.log.warn('[%s] No command type defined.', accessory.displayName);
      ...
      accessory.control.on('CurrentCustomAreaValues', (newVal) => {
        if (newVal && accessory.context.enableDebugLogging) {
          this.log('[%s] CurrentCustomAreaValues: %s.', accessory.displayName, JSON.stringify(newVal));
        }
      });
    ...
    try {
      ...
      if (accessory.context.enableDebugLogging) {
        this.log(
          '[%s] %s [GetCleanState%s].',
          accessory.displayName,
          platformLang.sendCmd,
          accessory.context.commandSuffix,
        );
      }
  ...
  async internalCleanUpdate(accessory, value) {
    try {
      ...
      const eText = parseError(err, [platformLang.errNotInit, platformLang.errNotReady]);
  ...
  async internalPredefinedAreaUpdate(accessory, value) {
    try {
      this.log.warn('[%s] initial value [%s] [%s]', accessory.displayName, value, typeof value);
      // Don't continue if we can't send commands to the device
      if (!accessory.control) {
        throw new Error(platformLang.errNotInit);
      }
      ...
      this.log.warn('[%s] rounded value [%s] [%s]', accessory.displayName, value, typeof value);

      // A value of 0 doesn't do anything
      if (value === 0) {
        this.log.warn('[%s] returning as value === 0', accessory.displayName);
        return;
      }
      ...
      if (updateKey !== accessory.context.lastCommandKey) {
        this.log.warn('[%s] skipping old value as new value noticed', accessory.displayName, value, typeof value);
        return;
      }

      // Obtain the commandType from the device config
      const commandType = this.deviceConf[accessory.context.ecoDeviceId][`commandType${value}`];

      // Don't continue if no command type for this number has been configured
      if (!commandType) {
        this.log.warn('[%s] returning no command type for predefined area %s.', accessory.displayName, value);
        return;
      }

      if (accessory.context.enableLogging) {
        this.log('[%s] Command type for predefined area %s is "%s"', accessory.displayName, value, commandType);
      }

      // Obtain the command from the device config
      const command = this.deviceConf[accessory.context.ecoDeviceId][commandType === 'spotArea' ? `spotAreaCommand${value}` : `customAreaCommand${value}`];

      // Don't continue if no command for this number has been configured
      if (!command) {
        this.log.warn('[%s] returning no command for predefined area %s', accessory.displayName, value);
        return;
      }

      if (accessory.context.enableLogging) {
        this.log('[%s] Command for predefined area %s is "%s"', accessory.displayName, value, command);
      }

      // TODO: remove?
      this.log.warn('Old log output: [%s] command [%s] [%s]', accessory.displayName, command, typeof command);

      // TODO: Remove?
      // Log the update if appropriate
      if (accessory.context.enableLogging) {
        this.log('Old Log output: [%s] %s [%s].', accessory.displayName, platformLang.curArea, value);
      }
      ...
      switch (commandType) {
        case 'spotArea':
          if (accessory.context.enableDebugLogging) {
            this.log('[%s] %s [SpotArea%s: %s].', accessory.displayName, platformLang.sendCmd, accessory.context.commandSuffix, command);
          }
          ...
          this.log.warn('[%s] command was sent', accessory.displayName);
          break;

        case 'customArea':
          if (accessory.context.enableDebugLogging) {
            this.log('[%s] %s [CustomArea%s: %s].', accessory.displayName, platformLang.sendCmd, accessory.context.commandSuffix, command);
          }
        ...
        default:
          this.log.warn('[%s] Unknown command type for predefined area: "%s".', commandType);
          break;
      }
      ...
      setTimeout(() => {
        ...
        this.log.warn('[%s] characteristic values reset to 0', accessory.displayName);
      }, 2000);
      this.log.warn('[%s] end of handler', accessory.displayName);
    } catch (err) {
      // Catch any errors during the process
      const eText = parseError(err, [platformLang.errNotInit, platformLang.errNotReady]);
      this.log.warn('[%s] %s %s.', accessory.displayName, platformLang.speedFail, eText);
