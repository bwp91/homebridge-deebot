...
import deebotClient from 'ecovacs-deebot';
...
export default class {
    ...
    Object.entries(config).forEach((entry) => {
      ...
      switch (key) {
        case 'countryCode':
          ...
          if (!platformConsts.validCCodes.includes(this.config.countryCode)) {
            throw new Error(platformLang.invalidCCode);
          }
          break;
        case 'debug':
  ...
  initialiseDevice(device) {
    try {
      ...
      this.deviceConf[device.did] = platformFuncs.hasProperty(this.deviceConf, device.did)
        ? this.deviceConf[device.did]
        : platformConsts.defaultDevice;
