...
export default class {
  ...
  initialiseDevice(device) {
    try {
      ...
      if (this.isBeta) {
        accessory.context.rawConfig.overrideLogging = 'debug';
      }
      switch (accessory.context.rawConfig.overrideLogging) {
        case 'standard':
      ...
      if (accessory.context.rawConfig.areaType1 || accessory.context.rawConfig.command1) {
  ...
  refreshAccessory(accessory) {
    try {
      ...
      accessory.log(`${platformLang.sendCmd} [GetNetInfo]`);
  ...
  async internalCleanUpdate(accessory, value) {
    try {
      ...
      accessory.logWarn(`${platformLang.cleanFail} ${eText}`);
  ...
  async internalSpeedUpdate(accessory, value) {
    try {
      ...
      // Catch any errors during the process
      const eText = parseError(err, [platformLang.errNotInit, platformLang.errNotReady]);
      accessory.logWarn(`${platformLang.speedFail} ${eText}`);
  ...
  async internalPredefinedAreaUpdate(accessory, value) {
    try {
      ...
      const oldCommand = accessory.context.rawConfig[`command${value}`];
      if (oldCommand) {
        accessory.logWarn(`${platformLang.oldCommandFound} ${value}: ${oldCommand}`);
      }

      // Obtain the area type from the device config or infer it from the old command respectively.
      const areaType = !oldCommand ? accessory.context.rawConfig[`areaType${value}`] : 'spotArea';
      ...
      // Obtain the command (<= release 5.1.0) from the device config or take over the old command respectively.
      const command = !oldCommand
        ? accessory.context.rawConfig[areaType === 'spotArea'
          ? `spotAreaIDs${value}`
  ...
  async internalChargeUpdate(accessory, value) {
    try {
      ...
      // Catch any errors during the process
      const eText = parseError(err, [platformLang.errNotInit, platformLang.errNotReady]);
      accessory.logWarn(`${platformLang.chargeFail} ${eText}`);
  ...
  async internalAirDryingUpdate(accessory, value) {
    try {
      ...
      // Catch any errors during the process
      const eText = parseError(err, [platformLang.errNotInit, platformLang.errNotReady]);
      accessory.logWarn(`${platformLang.airDryingFail} ${eText}`);
