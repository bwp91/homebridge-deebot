...
export default class {
  ...
  async internalPredefinedAreaUpdate(accessory, value) {
    try {
      ...
      if (value === 0) {
        this.log.warn('[%s] %s', accessory.displayName, platformLang.returningAsValueNull);
        return;
      }
      ...
      if (updateKey !== accessory.context.lastCommandKey) {
        this.log.warn('[%s] %s', accessory.displayName, platformLang.skippingValue);
        return;
      }

      // Check if there's defined an old command (<= release 5.1.0), which has priority for backwards compatibility reasons.
      const oldCommand = this.deviceConf[accessory.context.ecoDeviceId][`command${value}`];
      if (oldCommand) {
        this.log.warn('[%s] %s %s: %s', accessory.displayName, platformLang.oldCommandFound, value, oldCommand);
      }

      // Obtain the commandType from the device config or infer it from the old command repectively.
      const commandType = !oldCommand ? this.deviceConf[accessory.context.ecoDeviceId][`commandType${value}`] : 'spotArea';

      // Don't continue if no command type for this number has been configured
      if (!commandType) {
        this.log.warn('[%s] %s %s', accessory.displayName, platformLang.noCommandTypeForArea, value);
        return;
      }

      if (accessory.context.enableLogging) {
        this.log('[%s] %s %s: %s', accessory.displayName, platformLang.commandTypeForArea, value, commandType);
      }

      // Obtain the command (<= release 5.1.0) from the device config or take over the old command repectively.
      const command = !oldCommand ? this.deviceConf[accessory.context.ecoDeviceId][commandType === 'spotArea' ? `spotAreaCommand${value}` : `customAreaCommand${value}`] : oldCommand;

      // Don't continue if no command for this number has been configured
      if (!command) {
        this.log.warn('[%s] %s %s', accessory.displayName, platformLang.noCommandForArea, value);
        return;
      }

      if (accessory.context.enableLogging) {
        this.log('[%s] %s %s: %s', accessory.displayName, platformLang.commandForArea, value, command);
      }
      ...
      switch (commandType) {
        case 'spotArea':
          ...
          this.log.warn('[%s] %s', accessory.displayName, platformLang.commandSent);
          break;
        ...
        default:
          this.log.warn('[%s] %s: %s', commandType, platformLang.unknownCommandTypeForArea);
          break;
      }
      ...
      setTimeout(() => {
        ...
        this.log.warn('[%s] %s', accessory.displayName, platformLang.characteristicsReset);
      }, 2000);
      this.log.warn('[%s] %s', accessory.displayName, platformLang.endOfHandler);
    } catch (err) {
      // Catch any errors during the process
      const eText = parseError(err, [platformLang.errNotInit, platformLang.errNotReady]);
      this.log.warn('[%s] %s %s.', accessory.displayName, platformLang.speedFail, eText);
