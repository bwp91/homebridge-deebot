...
export default class {
              ...
              entries.forEach((subEntry) => {
                ...
                switch (k) {
                  ...
                  case 'showAirDryingSwitch':
                    if (typeof v !== 'string' || v === '' || !(v === 'presetting' || v === 'yes' || v === 'no')) {
                      logIgnore(`${key}.${id}.${k}`);
                    } else {
                      this.deviceConf[id][k] = v;
                    }
  ...
  initialiseDevice(device) {
    try {
      ...
      if (this.deviceConf[device.did].areaType1 || this.deviceConf[device.did].command1) {
      ...
      if (this.deviceConf[device.did].showAirDryingSwitch === 'yes' || (this.deviceConf[device.did].showAirDryingSwitch === 'presetting' && loadedDevice.hasAirDrying())) {
        if (!accessory.getService('Air Drying')) {
          accessory.addService(this.hapServ.Switch, 'Air Drying', 'airdrying');
          this.log('[%s] Air drying service added.', accessory.displayName);
        }
        ...
        this.log('[%s] Air drying service removed.', accessory.displayName);
      } else {
        this.log('[%s] No air drying available or not configured.', accessory.displayName);
      }
  ...
  refreshAccessory(accessory) {
    try {
      ...
      if (accessory.getService('Air Drying')) {
        if (accessory.context.enableDebugLogging) {
          this.log('[%s] %s [GetAirDrying].', accessory.displayName, platformLang.sendCmd);
        }
  ...
  async internalPredefinedAreaUpdate(accessory, value) {
    try {
      ...
      // Obtain the area type from the device config or infer it from the old command repectively.
      const areaType = !oldCommand ? this.deviceConf[accessory.context.ecoDeviceId][`areaType${value}`] : 'spotArea';

      // Don't continue if no command type for this number has been configured
      if (!areaType) {
        this.log.warn('[%s] %s %s', accessory.displayName, platformLang.noTypeForArea, value);
        return;
      }

      if (accessory.context.enableLogging) {
        this.log('[%s] %s %s: %s', accessory.displayName, platformLang.typeForArea, value, areaType);
      }

      // Obtain the command (<= release 5.1.0) from the device config or take over the old command repectively.
      const command = !oldCommand ? this.deviceConf[accessory.context.ecoDeviceId][areaType === 'spotArea' ? `spotAreaIDs${value}` : `customAreaCoordinates${value}`] : oldCommand;
      ...
      switch (areaType) {
        ...
        default:
          this.log.warn('[%s] %s: %s', areaType, platformLang.unknownCommandTypeForArea);
          break;
      }
  ...
  async internalAirDryingUpdate(accessory, value) {
    try {
      ...
      // Catch any errors during the process
      const eText = parseError(err, [platformLang.errNotInit, platformLang.errNotReady]);
      this.log.warn('[%s] %s %s.', accessory.displayName, platformLang.airDryingFail, eText);
  ...
  externalAirDryingUpdate(accessory, newVal) {
    try {
      // Log the received update if enabled
      if (accessory.context.enableDebugLogging) {
        this.log('[%s] %s [AirDryingState: %s].', accessory.displayName, platformLang.receiveCmd, newVal);
      }
      ...
      if (accessory.context.cacheAirDrying !== newVal) {
        ...
        // Log the change if appropriate
        if (accessory.context.enableLogging) {
          this.log('[%s] %s [%s].', accessory.displayName, platformLang.curAirDrying, newVal);
        }
      }
      ...
      // Catch any errors during the process
      const eText = parseError(err);
      this.log.warn('[%s] %s %s.', accessory.displayName, platformLang.inAirFail, eText);
    }
  }
