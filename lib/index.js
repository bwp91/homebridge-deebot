...
class DeebotPlatform {
  constructor (log, config, api) {
    ...
    try {
      ...
      const hideErrorLines = [this.messages.invalidCCode, this.messages.missingCreds]
      const eText = hideErrorLines.includes(err.message)
        ? err.message
    ...
    for (const [key, val] of Object.entries(config)) {
      switch (key) {
        ...
        case 'lowBattThreshold':
          this.config.lowBattThreshold = this.funcs.parseNumber(
            val,
            this.config.lowBattThreshold,
            this.consts.minValues.lowBattThreshold
          )
          break
        case 'motionDuration':
          this.config.motionDuration = this.funcs.parseNumber(
            val,
            this.config.motionDuration,
            this.consts.minValues.motionDuration
          )
          break
        case 'name':
        ...
        case 'showBattHumidity':
          this.config.showBattHumidity = !!val
          break
        case 'showMotionLowBatt':
          this.config.showMotionLowBatt = !!val
          break
        case 'username':
          this.config.username = this.funcs.parseUsername(val)
          break
        default:
