...
class DeebotPlatform {
  constructor (log, config, api) {
    ...
    if (!this.helpers.validCCodes.includes(this.countryCode)) {
      log.warn('*** Disabling plugin [v%s] ***', this.version)
      log.warn('*** Invalid ECOVACS country code. ***')
      return
    }
  ...
  async deebotSetup () {
    ...
    try {
      ...
      this.log.warn('*** Disabling plugin [v%s] ***', this.version)
      this.log.warn('Please restart Homebridge/HOOBS to try again.')
  ...
  configureAccessory (accessory) {
    ...
    try {
      ...
      cleanService.getCharacteristic(this.Characteristic.On)
        .on('set', (value, callback) => this.internalCleanUpdate(accessory, value, callback))
      chargeService.getCharacteristic(this.Characteristic.On)
  ...
  async externalMessageUpdate (accessory, msg) {
    if (accessory.context.inUse || this.helpers.messagesToIgnore.includes(msg)) {
      return
    }
  ...
  async externalErrorUpdate (accessory, err) {
    if (accessory.context.inUse || this.helpers.messagesToIgnore.includes(err)) {
      return
    }
    accessory.context.inUse = true
