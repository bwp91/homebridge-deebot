...
class DeebotPlatform {
  ...
  async pluginSetup () {
    ...
    try {
      ...
      this.log.warn(JSON.stringify(deviceList))
  ...
  initialiseDevice (device) {
    try {
      ...
      this.log.error(JSON.stringify(loadedDevice))

      // Get the cached accessory or add to Homebridge if doesn't exist
      const accessory = devicesInHB.get(uuid) || this.addAccessory(device.did, loadedDevice)
  ...
  addAccessory (deviceId, device) {
    // Add an accessory to Homebridge
    let displayName
    try {
      displayName = device.vacuum.nick || deviceId
      ...
      accessory
        ...
        .setCharacteristic(this.hapChar.SerialNumber, deviceId)
      ...
      accessory.context.ecoDeviceId = deviceId
