...
class DeebotPlatform {
  ...
  async pluginSetup () {
    ...
    try {
      ...
      try {
        ...
        if (e.message && e.message.includes('1010')) {
  ...
  initialiseDevice (device) {
    try {
      ...
      // Store the deviceId in a shorter variable name
      const deviceId = accessory.context.ecoDeviceId
      ...
      if (!accessory.getService('Attention') && !this.devices[deviceId].hideMotionSensor) {
      ...
      if (!accessory.getService('Battery Level') && this.devices[deviceId].showBattHumidity) {
      ...
      accessory.control.on('CleanSpeed', newStatus => {
        this.externalSpeedUpdate(accessory, newStatus)
      })
      ...
      accessory.control.on('NetInfoIP', newStatus => {
        this.externalIPUpdate(accessory, newStatus)
      })

      // Set up a listener for the device 'NetInfoMAC' event
      accessory.control.on('NetInfoMAC', newStatus => {
        this.externalMacUpdate(accessory, newStatus)
      })
  ...
  externalCleanUpdate (accessory, newStatus) {
    try {
      ...
      if (accessory.cacheClean !== newStatus) {
        ...
        accessory.getService('Clean').updateCharacteristic(this.hapChar.On, newStatus === 'auto')

        // Log the change if appropriate
        if (this.config.debug || !this.config.disableDeviceLogging) {
          this.log('[%s] %s [%s].', accessory.displayName, this.lang.curCleaning, newStatus)
        }
      }
  ...
  externalSpeedUpdate (accessory, newStatus) {
    try {
      // Check if the new cleaning state is different from the cached state
      if (accessory.cacheSpeed !== newStatus) {
        // State is different so update service
        accessory
          .getService('Clean')
          .updateCharacteristic(this.cusChar.MaxSpeed, [3, 4].includes(newStatus))
        ...
        if (this.config.debug || !this.config.disableDeviceLogging) {
          this.log(
            ...
            this.lang.curSpeed,
            this.consts.speed2Label[newStatus]
          )
        }
      }

      // Always update the cache with the new cleaning status
      accessory.cacheSpeed = newStatus
    } catch (err) {
      // Catch any errors during the process
      const eText = this.funcs.parseError(err)
  ...
  externalChargeUpdate (accessory, newStatus) {
    try {
      ...
      if (accessory.cacheCharge !== newStatus) {
        ...
        accessory
          .getService(this.hapServ.BatteryService)
          .updateCharacteristic(this.hapChar.ChargingState, chargeState)

        // Log the change if appropriate
        if (this.config.debug || !this.config.disableDeviceLogging) {
          this.log('[%s] %s [%s].', accessory.displayName, this.lang.curCharging, newStatus)
        }
      }
  ...
  externalIPUpdate (accessory, newStatus) {
    try {
      // Check if the new charging state is different from the cached state
      if (accessory.context.ipAddress !== newStatus) {
        // State is different so update context info
        accessory.context.ipAddress = newStatus
  ...
  externalMacUpdate (accessory, newStatus) {
    try {
      // Check if the new charging state is different from the cached state
      if (accessory.context.macAddress !== newStatus) {
        // State is different so update context info
        accessory.context.macAddress = newStatus
