...
class DeebotPlatform {
  constructor (log, config, api) {
    ...
    try {
      ...
      this.motionDuration = isNaN(this.motionDuration) || this.motionDuration < 1
        ...
        : this.motionDuration
      this.parsedIgnoredDevices = []

      // *** Backwards Compatibility *** \\
      if (typeof config.ignoredDevices === 'string' && config.ignoredDevices.length > 0) {
        config.ignoredDevices = config.ignoredDevices.split(',')
      }
      // *** End *** \\

      if (Array.isArray(config.ignoredDevices) && config.ignoredDevices.length > 0) {
        config.ignoredDevices.forEach(sn => {
          this.parsedIgnoredDevices.push(sn.replace(/[\s'"]+/g, '').toUpperCase())
        })
      }
  ...
  async deebotSetup () {
    try {
      ...
      this.ecovacsAPI = new this.deebotClient.EcoVacsAPI(
        this.deebotClient.EcoVacsAPI.getDeviceId((require('node-machine-id')).machineIdSync()),
        this.countryCode,
        this.deebotClient.countries[this.countryCode].continent
      )
      ...
      devices.forEach(d => this.initialiseDevice(d))
  ...
  deebotShutdown () {
    try {
      ...
      this.devicesInHB.forEach(a => {
        if (a.control && a.control.is_ready) {
          a.control.disconnect()
        }
      })
    } catch (err) {}
  }
  ...
  initialiseDevice (device) {
    try {
      if (this.parsedIgnoredDevices.includes(device.did)) {
      ...
      this.log('[%s] initialised with id %s.', accessory.displayName, device.did)
      ...
      this.log.warn('[%s] could not be initialised as%s', device.did, errToShow)
    }
  }
  ...
  addAccessory (device) {
    ...
    try {
      ...
      const errToShow = this.debug
        ? ':\n' + err
        : ' ' + err.message + (err.lineNumber ? ' [line ' + err.lineNumber + '].' : '')
      this.log.warn('[%s] could not be added to Homebridge as%s', displayName, errToShow)
    }
  }
  ...
  async internalCleanUpdate (accessory, value, callback) {
    try {
      ...
      const errToShow = this.debug
        ? ':\n' + err
        : ' ' + err.message + (err.lineNumber ? ' [line ' + err.lineNumber + '].' : '')
      this.log.warn('[%s] sending clean update failed as%s', accessory.displayName, errToShow)
    }
  }
  ...
  async internalChargeUpdate (accessory, value, callback) {
    try {
      ...
      const errToShow = this.debug
        ? ':\n' + err
        : ' ' + err.message + (err.lineNumber ? ' [line ' + err.lineNumber + '].' : '')
      this.log.warn('[%s] sending charge update failed as%s', accessory.displayName, errToShow)
    }
  }
  ...
  externalReadyUpdate (accessory, event) {
    try {
      ...
      const errToShow = this.debug
        ? ':\n' + err
        : ' ' + err.message + (err.lineNumber ? ' [line ' + err.lineNumber + '].' : '')
      this.log.warn('[%s] receiving ready update failed as%s', accessory.displayName, errToShow)
    }
  }
  ...
  externalCleanUpdate (accessory, newStatus) {
    try {
      ...
      const errToShow = this.debug
        ? ':\n' + err
        : ' ' + err.message + (err.lineNumber ? ' [line ' + err.lineNumber + '].' : '')
      this.log.warn('[%s] receiving clean update failed as%s', accessory.displayName, errToShow)
    }
  }
  ...
  externalChargeUpdate (accessory, newStatus) {
    try {
      ...
      const errToShow = this.debug
        ? ':\n' + err
        : ' ' + err.message + (err.lineNumber ? ' [line ' + err.lineNumber + '].' : '')
      this.log.warn('[%s] receiving charge update failed as%s', accessory.displayName, errToShow)
    }
  }
  ...
  externalBatteryUpdate (accessory, newVal) {
    try {
      ...
      const errToShow = this.debug
        ? ':\n' + err
        : ' ' + err.message + (err.lineNumber ? ' [line ' + err.lineNumber + '].' : '')
      this.log.warn('[%s] receiving battery update failed as%s', accessory.displayName, errToShow)
    }
  }
  ...
  async externalMessageUpdate (accessory, msg) {
    try {
      ...
      const errToShow = this.debug
        ? ':\n' + err
        : ' ' + err.message + (err.lineNumber ? ' [line ' + err.lineNumber + '].' : '')
      this.log.warn('[%s] receiving message update failed as%s', accessory.displayName, errToShow)
    }
  }
  ...
  async externalErrorUpdate (accessory, err) {
    try {
      ...
      const errToShow = this.debug
        ? ':\n' + err
        : ' ' + err.message + (err.lineNumber ? ' [line ' + err.lineNumber + '].' : '')
      this.log.warn('[%s] receiving error update failed as%s', accessory.displayName, errToShow)
    }
  }
}
