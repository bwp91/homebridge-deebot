...
class DeebotPlatform {
  ...
  initialiseDevice (device) {
    try {
      ...
      switch (this.devices[device.did].overrideLogging) {
        case 'standard':
      ...
      if (!accessory.getService('Attention') && !this.devices[device.did].hideMotionSensor) {
      ...
      if (!accessory.getService('Battery Level') && this.devices[device.did].showBattHumidity) {
      ...
      this.log(
        ...
        JSON.stringify(this.devices[device.did])
      )
  ...
  async externalMessageUpdate (accessory, msg) {
    try {
      ...
      }, this.devices[accessory.context.ecoDeviceId].motionDuration * 1000)
    } catch (err) {
