  ...
  async pluginSetup () {
    ...
    try {
      ...
      this.ecovacsAPI = new this.deebotClient.EcoVacsAPI(
        this.deebotClient.EcoVacsAPI.getDeviceId(
          (require('node-machine-id')).machineIdSync()
        ),
  ...
  externalReadyUpdate (accessory, event) {
    try {
      ...
      accessory.ordersToSend.forEach(order => {
        ...
        if (this.config.debug) {
          this.log('[%s] %s [%s].', accessory.displayName, this.messages.sendCmd, order)
        }
      })
