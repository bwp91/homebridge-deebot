...
class DeebotPlatform {
  ...
  async pluginSetup () {
    ...
    try {
      ...
      // Setup a (hopefully) constant uuid for the ecovacs-deebot client
      const nonce = this.api.hap.uuid.generate(this.config.username)

      // Connect to ECOVACS
      this.ecovacsAPI = new deebotClient.EcoVacsAPI(
        deebotClient.EcoVacsAPI.getDeviceId(nonce),
      ...
      this.log('[%s] %s.', deviceList.length, this.lang.deviceCount)
  ...
  initialiseDevice (device) {
    try {
      ...
      cleanService
        ...
        .removeOnSet()
        .onSet(async value => await this.internalCleanUpdate(accessory, value))
      ...
      chargeService
        ...
        .removeOnSet()
        .onSet(async value => await this.internalChargeUpdate(accessory, value))
      ...
      if (accessory.context.enableDebugLogging) {
        accessory.control.on('Maps', maps => {
          ...
          for (const i in maps.maps) {
            if (this.funcs.hasProperty(maps.maps, i)) {
        ...
        accessory.control.on('MapSpotAreas', spotAreas => {
          ...
          for (const i in spotAreas.mapSpotAreas) {
            if (this.funcs.hasProperty(spotAreas.mapSpotAreas, i)) {
  ...
  async internalCustomAreaUpdate (accessory, value) {
    try {
      ...
      // Log the update if appropriate
      if (accessory.context.enableLogging) {
        this.log('[%s] %s [%s].', accessory.displayName, this.lang.curArea, value)
      }

      // Send the command
      if (accessory.context.enableDebugLogging) {
        this.log('[%s] %s [SpotArea: %s].', accessory.displayName, this.lang.sendCmd, command)
      }
