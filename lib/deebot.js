...
class Deebot {
  constructor(log, config, api) {
    ...
    if (!config.username || !config.password || !config.countryCode) {
      log.error(
        "**************** Cannot load homebridge-deebot *****************"
      );
      log.error(
        "Your ECOVACS credentials are missing from the Homebridge config."
      );
      log.error(
        "****************************************************************"
      );
    ...
    this.api
      .on("didFinishLaunching", () => {
        this.log(
          "Plugin has finished initialising. Starting synchronisation with ECOVACS account."
        );
        let getDeebots = () => {
          this.ecovacsAPI
            .connect(
              this.config.username,
              deebot.EcoVacsAPI.md5(this.config.password)
            )
            .then(() => this.ecovacsAPI.devices())
            .then(devices => {
              ...
              if (this.devicesInEC.size === 0) {
                this.log.warn(
                  "No Deebot devices were found. Will try discovery again in 1 minute."
                );
              ...
              this.refreshTimer =
                this.config.ref >= 30 && this.config.ref <= 600
                  ? this.config.ref
                  : 300;
            ...
            .catch(err => {
              if (err.message === "incorrect email or password") {
                ...
                log.error(
                  "Your ECOVACS credentials are incorrect. Please double check."
                );
                log.error(
                  "************************************************************"
                );
                ...
                this.log.warn(
                  "Could not retrieve Deebot devices. Will try discovery again in 1 minute.\n%s",
                  err
                );
                setTimeout(() => getDeebots(), 60000);
              }
  ...
  initialiseDevice(device) {
    ...
    if ((accessory = this.devicesInHB.get(device.vacuum.did))) {
      ...
      device.on("BatteryInfo", newValue => {
        let oldValue = accessory
          .getService(Service.BatteryService)
          .getCharacteristic(Characteristic.BatteryLevel).value;
        ...
        if (oldValue !== newValue) {
          accessory
            .getService(Service.BatteryService)
            .updateCharacteristic(Characteristic.BatteryLevel, newValue);
          accessory
            .getService(Service.BatteryService)
        ...
        if (this.debug) {
          this.log(
            "[%s] battery level updated to from [%s%] to [%s%].",
            displayName,
            oldValue,
            newValue
          );
      ...
      device.on("ChargeState", newStatus => {
        if (this.debug) {
          this.log(
            "[%s] updated ChargeState status is [%s].",
            displayName,
            newStatus
          );
        }
        ...
        accessory
          ...
          .updateCharacteristic(Characteristic.On, newStatus === "returning");
      });
      device.on("CleanReport", newStatus => {
        if (this.debug) {
          this.log(
            "[%s] updated CleanReport status is [%s].",
            displayName,
            newStatus
          );
        }
        if (newStatus) {
          accessory
            .getService("Clean")
            .updateCharacteristic(Characteristic.On, newStatus === "auto");
        }
      });
      ...
      device.on("Error", err => {
        accessory
          ...
          .getCharacteristic(Characteristic.MotionDetected, true);
        accessory
          .getService("Clean")
        ...
        setTimeout(() => {
          accessory
            .getService("Attention")
            .getCharacteristic(Characteristic.MotionDetected, false);
        }, 10000);
  ...
  addAccessory(device) {
    ...
    try {
      const accessory = new Accessory(
        displayName,
        UUIDGen.generate(displayName).toString()
      );
      accessory
        ...
        .on("set", (value, callback) => {
          ...
          setTimeout(() => {
            if (
              !accessory
                .getService(Service.BatteryService)
                .getCharacteristic(Characteristic.ChargingState).value
            ) {
