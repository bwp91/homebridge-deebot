...
class Deebot {
  constructor (log, config, api) {
    ...
    this.ecovacsAPI = new deebot.EcoVacsAPI(deebot.EcoVacsAPI.md5(nodeMachineId.machineIdSync()), this.config.countryCode, deebot.countries[this.config.countryCode].continent.toUpperCase())
    this.api.on('didFinishLaunching', () => {
      this.log('Plugin has finished initialising. Starting synchronisation with ECOVACS account.')
      const getDeebots = () => {
        this.ecovacsAPI.connect(this.config.username, deebot.EcoVacsAPI.md5(this.config.password)).then(() => this.ecovacsAPI.devices()).then(devices => {
          devices.forEach(device => this.devicesInEC.set(device.did, this.ecovacsAPI.getVacBot(this.ecovacsAPI.uid, deebot.EcoVacsAPI.REALM, this.ecovacsAPI.resource, this.ecovacsAPI.user_access_token, device, deebot.countries[this.config.countryCode].continent.toUpperCase())))
          if (this.devicesInEC.size === 0) {
          ...
          if (err.message === 'incorrect email or password') {
            log.error('************** Cannot load homebridge-deebot ***************')
            log.error('Your ECOVACS credentials are incorrect. Please double check.')
            log.error('************************************************************')
          } else {
  ...
  addAccessory (device) {
    ...
    try {
      ...
      accessory.getService(Service.AccessoryInformation).setCharacteristic(Characteristic.SerialNumber, device.vacuum.did).setCharacteristic(Characteristic.Manufacturer, device.vacuum.company).setCharacteristic(Characteristic.Model, device.deviceModel).setCharacteristic(Characteristic.Identify, false)
      ...
      this.api.registerPlatformAccessories('homebridge-deebot', 'Deebot', [
        accessory
      ])
  ...
  configureAccessory (accessory) {
    if (!this.log) return
    try {
      accessory.getService(Service.BatteryService).getCharacteristic(Characteristic.BatteryLevel).on('get', callback => {
        if (accessory.deebot) {
      ...
      accessory.getService('Clean').getCharacteristic(Characteristic.On).on('get', callback => {
        setTimeout(() => {
          if (accessory.deebot) {
            if (accessory.deebot.is_ready) {
      ...
      accessory.getService('Go Charge').getCharacteristic(Characteristic.On).on('set', (value, callback) => {
        callback()
        setTimeout(() => {
      ...
      accessory.getService('Attention').getCharacteristic(Characteristic.MotionDetected).on('get', callback => callback())
