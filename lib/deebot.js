                        ...
                        .then(devices => {
                           ...
                           this.timer = setInterval(() => {
                              this.devicesInHB.forEach(a => {
                                 if (a.vacBot && a.vacBot.is_ready) {
                                    a.vacBot.run("GetBatteryState");
                                    a.vacBot.run("GetChargeState");
                                    a.vacBot.run("GetCleanState");
                                 } else {
                                    a.vacBot.connect_and_wait_until_ready();
                                 }
                              });
   ...
   initialiseDevice(device) {
      ...
      if ((accessory = this.devicesInHB.get(device.vacuum.did))) {
         ...
         device.on("ChargeState", newStatus => {
            let charging = newStatus === "charging",
               idle = newStatus === "idle",
               returning = newStatus === "returning";
            if (accessory.getService(Service.BatteryService).getCharacteristic(Characteristic.ChargingState).value !== charging) {
               accessory.getService(Service.BatteryService).updateCharacteristic(Characteristic.ChargingState, charging);
            }
            if (this.debug) {
               this.log("[%s] charging status is [charging: %s, idle: %s, returning: %s].", displayName, charging, idle, returning);
            }
         });
         device.on("CleanReport", newStatus => {
            if (newStatus) {
               accessory.getService("Clean").updateCharacteristic(Characteristic.On, !["stop", "pause", "idle"].includes(newStatus));
            }
            if (this.debug) {
               this.log("[%s] cleaning status is [%s].", displayName, newStatus);
            }
         });
         ...
         device.on("Error", err => {
            this.log.warn("[%s] has sent an error message - %s.", displayName, err);
   ...
   configureAccessory(accessory) {
      ...
      try {
         accessory.getService(Service.BatteryService).getCharacteristic(Characteristic.BatteryLevel)
            .on("get", callback => {
               if (accessory.deebot && accessory.deebot.is_ready) {
         ...
         accessory.getService("Clean").getCharacteristic(Characteristic.On)
            .on("get", callback => {
               ...
               callback(undefined, accessory.getService("Clean").getCharacteristic(Characteristic.On).value);
            })
            .on("set", (value, callback) => {
               let order = value ? "auto" : "Stop";
               if (accessory.deebot && accessory.deebot.is_ready) {
         ...
         accessory.getService("Go Charge").getCharacteristic(Characteristic.On)
            .on("set", (value, callback) => {
               if (value) {
                  if (accessory.deebot && accessory.deebot.is_ready) {
                     accessory.deebot.run.apply(accessory.deebot, "Charge");
                  } else {
                     accessory.deebot.orderToSend = "Charge";
                     accessory.deebot.connect_and_wait_until_ready();
                  }
                  setTimeout(() => {
