...
class Deebot {
  ...
  async deebotSync () {
    try {
      ...
      this.devicesInEC.forEach(d => this.refreshDevice(d))
    } catch (err) {
  ...
  async refreshDevice (device) {
    try {
      ...
      device.on('ready', event => this.externalReadyUpdate(accessory, event))
      device.on('BatteryInfo', newValue => this.externalBatteryUpdate(accessory, newValue))
      device.on('ChargeState', newStatus => this.externalChargeUpdate(accessory, newStatus))
      device.on('CleanReport', newStatus => this.externalCleanUpdate(accessory, newStatus))
      device.on('message', msg => this.externalMessageUpdate(accessory, msg))
