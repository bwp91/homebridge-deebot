    ...
    const getDeebots = async () => {
      try {
        ...
        if (err.message === 'incorrect email or password') {
          this.log.error('************** Cannot load homebridge-deebot ***************')
          this.log.error('Your ECOVACS credentials are incorrect. Please double check.')
          this.log.error('************************************************************')
        } else {
          this.log.warn('Could not retrieve Deebot devices. Will try discovery again in 1 minute.\n%s', this.debug ? err : err.message)
          setTimeout(() => getDeebots(), 60000)
        }
      }
    }
    ...
    try {
      ...
      device.on('ready', event => {
        ...
        if (device.orderToSend) {
          if (this.debug) {
            this.log('[%s] sending command [%s].', accessory.displayName, device.orderToSend)
          }
      ...
      device.on('message', msg => {
        this.log('[%s] has sent a message - %s', accessory.displayName, msg)
      })
      device.on('Error', err => {
      ...
      this.log(' → [%s] initialised from your ECOVACS account.', accessory.displayName)
    } catch (err) {
