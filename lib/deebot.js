...
class Deebot {
   constructor(log, config, api) {
      ...
      this.login = this.config.email;
      this.password = this.config.password;
      this.countryCode = this.config.countryCode;
      this.refreshTimer = this.checkTimer(this.config.refreshTimer);
      this.deviceList = [];
      this.cleanCache = this.config.cleanCache;
      this.publishBipSwitch = this.checkParameter(this.config.publishBipSwitch, true);
      this.publishSwitch = this.checkParameter(this.config.publishSwitch, true);
      this.publishFan = this.checkParameter(this.config.publishFan, true);
      this.publishMotionDetector = this.checkParameter(this.config.publishMotionDetector, true);
      this.leftDirectionCleaningMode = this.checkParameter(this.config.leftDirectionCleaningMode, "edge");
      this.rightDirectionCleaningMode = this.checkParameter(this.config.rightDirectionCleaningMode, "auto");
      this.foundAccessories = [];
      this.apiClient = new deebotAPI(log, this);
      ...
      this.api.on("didFinishLaunching", () => {
         ...
         this.apiClient.getDeebots();
         this.apiClient.discoverSuccess(devices => {
         ...
         this.apiClient.discoverFailure(err => {
            this.log.error("Could not retrieve Deebot devices as %s.", err);
            setTimeout(() => {
               this.apiClient.getDeebots();
            }, 60000);
         });
   ...
   loadDeebots() {
      this.deviceList.forEach(device => {
         ...
         this.apiClient.configureEvents(device, HKBatteryService, HKFanService, HKSwitchOnService, HKMotionService, this);
