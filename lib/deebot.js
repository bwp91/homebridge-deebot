   ...
   initialiseDevice(device) {
      ...
      if ((accessory = this.devicesInHB.get(device.vacuum.did))) {
         ...
         device.on("ChargeState", newStatus => {
            ...
            if (this.debug) {
               this.log("[%s] updated status is [%s].", displayName, newStatus);
            }
         });
         device.on("CleanReport", newStatus => {
            ...
            if (this.debug) {
               this.log("[%s] updated status is [cleaning: %s].", displayName, newStatus);
            }
         });
      ...
      try {
         ...
         accessory.getService("Go Charge").getCharacteristic(Characteristic.On)
            .on("set", (value, callback) => {
               ...
               setTimeout(() => accessory.getService("Go Charge").updateCharacteristic(Characteristic.On, false), 2000);
            });
